...
def test_cannot_reserve_for_another_user_when_reservation_was_made_for_other_person(
    ...
    product_availability.reserve(
        for_=1, until=datetime.now(timezone.utc) + timedelta(hours=1)
    )

    with pytest.raises(ProductAlreadyReserved):
        product_availability.reserve(
            for_=2, until=datetime.now(timezone.utc) + timedelta(hours=1)
        )
...
def test_freed_product_can_be_reserved_again(
    ...
    product_availability.reserve(
        for_=1, until=datetime.now(timezone.utc) + timedelta(days=2)
    )
    product_availability.free()

    try:
        product_availability.reserve(
            for_=2, until=datetime.now(timezone.utc) + timedelta(days=2)
        )
    except ProductAlreadyReserved:
