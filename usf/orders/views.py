...
class OrderViewSet(ViewSet):
    ...
    def create(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        order = create_order_preview(product_id, user_id, DjangoOrderRepository())
    ...
    def confirm(self, request: Request, pk: int) -> Response:
        order = confirm(order_id=pk, repository=DjangoOrderRepository())
        serializer = OrderDtoSerializer(order)
        return Response(serializer.data)

    def list(self, request: Request) -> Response:
        user_id = request.user.id
        orders = get_all_orders(user_id, DjangoOrderRepository())
